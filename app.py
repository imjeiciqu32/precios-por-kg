...
# --- NUEVA FUNCIÓN PIRÁMIDE ---
def calcular_piramide_dinamica(df):
    if df.empty: return df
    df_temp = df.copy()
    # Identificar Precio Base 100 (Máximo SOM por Ocasión)
    idx_base = df_temp.groupby("Ocasión")["SOM (%)"].idxmax()
    precios_base = df_temp.loc[idx_base, ["Ocasión", "Precio por Kg ($)"]]
...
# --- 8. PIRÁMIDE DE MARCA (NUEVA SECCIÓN) ---
st.divider()
st.subheader("🏔️ Pirámide de Posicionamiento por Tier")

if not st.session_state.data.empty:
    df_piramide = calcular_piramide_dinamica(st.session_state.data)
    
    # Selector de ocasión para no mezclar formatos en la pirámide
    filtro_oca = st.selectbox("Seleccionar Segmento para Pirámide:", df_piramide["Ocasión"].unique())
    df_filtered = df_piramide[df_piramide["Ocasión"] == filtro_oca].sort_values("Tier_Order")

    # Definición de colores para los Tiers
    tier_colors = {
        "PREMIUM": "#1A237E",
    ...
    # Mostrar la pirámide por bloques
    for tier in ["PREMIUM", "UPPER MAINSTREAM", "MAINSTREAM", "MAINSTREAM LOW", "VALUE"]:
        ...
        if not productos_tier.empty:
            with st.container():
                ...
                c1.markdown(f"""
                    <div style="background-color:{tier_colors[tier]}; color:white; padding:10px; 
                    border-radius:5px; text-align:center; font-weight:bold; height:100%;">
                        {tier}
                    </div>
                """, unsafe_allow_html=True)
                ...
                for _, row in productos_tier.iterrows():
                    border_color = "#4B207E" if row["Fabricante"] == "BARCEL" else "#CCCCCC"
                    items_html += f"""
                        <div style="display:inline-block; border: 2px solid {border_color}; 
                        border-radius:10px; padding:10px; margin:5px; background:white; min-width:150px;">
                            <span style="font-size:0.9rem; font-weight:bold;">{row['Producto']}</span><br>
                            <span style="color:#666; font-size:0.8rem;">Index: {int(row['Brand_Index'])}</span><br>
                            <span style="font-size:1rem; font-weight:bold;">${int(row['Precio ($)'])} ({row['Gramaje (g)']}g)</span>
                        </div>
                    """
                c2.markdown(f'<div style="display:flex; flex-wrap:wrap;">{items_html}</div>', unsafe_allow_html=True)
                st.write("") # Espaciador
