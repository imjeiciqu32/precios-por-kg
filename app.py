...
# --- 14. SIMULADOR ESTRATÉGICO DIRECTIVO (V7: LÓGICA EXPERTA + FIX DUPLICATE ID) ---
if modo == "Price Ladder" and not st.session_state.data.empty:
    st.divider()
    ...
    st.markdown("""
        ...
        .ai-box {
            background-color: #f0f7ff; border-radius: 10px; padding: 20px;
            border: 1px dotted #002366; color: #002366;
        }
        </style>
    """, unsafe_allow_html=True)
    ...
    if not lista_comp.empty:
        # --- PARTE 1: CONFIGURACIÓN HORIZONTAL (FIX ID DUPLICADOS) ---
        st.markdown("##### ⚙️ Configuración del Escenario")
        c_in1, c_in2, c_in3, c_in4 = st.columns(4)
        
        with c_in1:
            comp_a_mover = st.selectbox("Benchmark Competidor:", lista_comp["Producto"].unique(), key="v7_bench_select")
            datos_comp = lista_comp[lista_comp["Producto"] == comp_a_mover].iloc[0]
            n_p_c = st.number_input(f"Nuevo Precio {comp_a_mover}:", min_value=1.0, value=float(datos_comp["Precio ($)"]), step=1.0, key="v7_p_comp")
        
        with c_in2:
            n_g_c = st.number_input(f"Gramaje {comp_a_mover} (g):", min_value=1, value=int(datos_comp["Gramaje (g)"]), step=1, key="v7_g_comp")
            pkg_c_nuevo = n_p_c / (n_g_c / 1000)
        ...
        with c_in3:
            ...
            df_barcel_oca = df_sim[(df_sim["Fabricante"] == "BARCEL") & (df_sim["Ocasión"] == oca_sim)]
            if not df_barcel_oca.empty:
                prod_b = st.selectbox("Producto Barcel:", df_barcel_oca["Producto"].unique(), key="v7_barcel_select")
                row_b = df_barcel_oca[df_barcel_oca["Producto"] == prod_b].iloc[0]
                n_p_b = st.number_input(f"Nuevo Precio {prod_b}:", min_value=1.0, value=float(row_b["Precio ($)"]), step=1.0, key="v7_p_barcel")
            else:
                st.warning(f"Sin Barcel en {oca_sim}"); st.stop()

        with c_in4:
            n_g_b = st.number_input(f"Gramaje {prod_b} (g):", min_value=1, value=int(row_b["Gramaje (g)"]), step=1, key="v7_g_barcel")
            pkg_b_nuevo = n_p_b / (n_g_b / 1000)

        # Cálculos de Variación e Index
        var_p_c = ((n_p_c / datos_comp["Precio ($)"]) - 1) * 100 if datos_comp["Precio ($)"] > 0 else 0
        var_pkg_c = ((pkg_c_nuevo / datos_comp["Precio por Kg ($)"]) - 1) * 100 if datos_comp["Precio por Kg ($)"] > 0 else 0
        var_p_b = ((n_p_b / row_b["Precio ($)"]) - 1) * 100 if row_b["Precio ($)"] > 0 else 0
        var_pkg_b = ((pkg_b_nuevo / row_b["Precio por Kg ($)"]) - 1) * 100 if row_b["Precio por Kg ($)"] > 0 else 0
        ...
        with c1:
            ...
            st.metric(f"{prod_b} vs {comp_a_mover}", f"Index {idx_des_nue}", f"{idx_des_nue - idx_des_ant} pts vs act.")
            st.caption(f"Var. Precio: Barcel {var_p_b:+.1f}% | Rival {var_p_c:+.1f}%")
            st.markdown('</div>', unsafe_allow_html=True)
        with c2:
            st.markdown('<div class="report-card">', unsafe_allow_html=True)
            st.write("⚖️ **EFICIENCIA (VALOR $/KG)**")
            st.metric(f"{prod_b} vs {comp_a_mover}", f"Index {idx_pkg_nue}", f"{idx_pkg_nue - idx_pkg_ant} pts vs act.")
            st.caption(f"Var. Eficiencia: Barcel {var_pkg_b:+.1f}% | Rival {var_pkg_c:+.1f}%")
            st.markdown('</div>', unsafe_allow_html=True)

        # --- BOTÓN CONSULTOR EXPERTO (LÓGICA ESTRATÉGICA) ---
        if st.button("🤖 Generar Executive Summary (Consultor Experto)"):
            with st.spinner("Analizando variables..."):
                # Determinación del Veredicto
                lider_occ = df_sim[df_sim["Ocasión"] == oca_sim].sort_values("SOM (%)", ascending=False).iloc[0]
                soy_lider = (lider_occ["Fabricante"] == "BARCEL")
                
                st.markdown('<div class="ai-box">', unsafe_allow_html=True)
                st.markdown("#### 📝 Análisis Estratégico de Portfolio")
                
                # Regla 1: Riesgo de Posicionamiento
                if not soy_lider and idx_des_nue > 100:
                    st.error(f"⚠️ **Riesgo Crítico:** Estás operando con un premium de precio ({idx_des_nue}) en una ocasión donde no eres líder. Riesgo inminente de pérdida de volumen frente a {comp_a_mover}.")
                elif idx_pkg_nue > 103:
                    st.warning(f"🔔 **Alerta de Valor:** Tu eficiencia $/Kg es inferior a la del rival. El consumidor percibe que paga más por menos producto.")
                else:
                    st.success(f"✅ **Posicionamiento Saludable:** El escenario mantiene a {prod_b} en un rango competitivo de paridad.")

                # Regla 2: Impacto de Reduflación
                if n_g_b < row_b["Gramaje (g)"]:
                    st.info(f"🔍 **Impacto de Gramaje:** La reducción de {int(row_b['Gramaje (g)'] - n_g_b)}g permite absorber el costo, pero degrada la eficiencia en un {var_pkg_b:.1f}%.")

                # Regla 3: Sell-story
                st.markdown("**💡 Sell-story sugerida:**")
                if idx_des_nue <= 100:
                    st.write(f"*'Mantenemos la paridad en el punto de precio clave para el consumidor, asegurando que {prod_b} siga siendo la opción de valor preferida en {oca_sim}.'*")
                else:
                    st.write(f"*'Ajustamos el posicionamiento premium de {prod_b} alineado a la propuesta de valor superior de la marca, gestionando la rentabilidad por gramo.'*")
                
                st.markdown('</div>', unsafe_allow_html=True)

        # Botón Snapshot
        if st.button("📸 Guardar Escenario en Historial"):
            st.session_state.snapshots.append({
                "ID": f"S{len(st.session_state.snapshots)+1}", "Barcel": prod_b, "Rival": comp_a_mover,
                "Precio B": n_p_b, "Gram B": n_g_b, "Idx Des.": idx_des_nue, "Idx $/Kg": idx_pkg_nue
            })
            st.toast("Escenario guardado")
        ...
        g_final = int(5 * round(((n_p_b / (pkg_c_nuevo * 0.92)) * 1000) / 5))
        ca, cb = st.columns(2)
        ca.info(f"**Opción A: Mantener {n_g_b}g**\n\nPrecio: **${int(p_tec)}** (Index 92)")
        cb.info(f"**Opción B: Mantener ${n_p_b}**\n\nContenido: **{g_final}g** (Index 92)")

    if st.session_state.snapshots:
        st.write("---")
        st.markdown("### 📋 Historial de Comparativa")
        st.table(pd.DataFrame(st.session_state.snapshots))
        if st.button("🗑️ Limpiar"):
            st.session_state.snapshots = []; st.rerun()
