...
import os
from plantillas import PLANTILLAS  # <--- Esta línea hace toda la magia
...
# --- LÓGICA DE PIRÁMIDE (PROCESAMIENTO) ---
def procesar_datos_piramide(df):
    ...
    bases = temp.groupby("Ocasión").apply(get_base).reset_index()
    bases.columns = ["Ocasión", "P_Ref"]
    temp = temp.merge(bases, on="Ocasión", how="left")
...
# --- 6. GRÁFICO FINAL (MANTENIDO EXACTAMENTE IGUAL) ---
...
# --- 8. PIRÁMIDE CORREGIDA (SOLUCIÓN AL ERROR DE DIVS) ---
st.divider()
...
if not df_p.empty:
    ...
    for tier in ["PREMIUM", "UPPER MAINSTREAM", "MAINSTREAM", "MAINSTREAM LOW", "VALUE"]:
        ...
        if not productos_tier.empty:
            ...
            # Construcción limpia del string de tarjetas
            cards_html = ""
            ...
            # Envolvemos todo en un contenedor padre para evitar que Streamlit corte el HTML
